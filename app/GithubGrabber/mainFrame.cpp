...
CDuiString TestFrame::GetSkinFile() { return _T("grabberMainFrame.xml"); }
...
void TestFrame::InitWindow() {
  ...
  m_boxOpened =
      (CCheckBoxUI *)m_PaintManager.FindControl(_T("RULE_WITH_OPENED"));
  m_boxClosed =
      (CCheckBoxUI *)m_PaintManager.FindControl(_T("RULE_WITH_CLOSED"));
  m_boxPortBug =
  ...
  m_boxCommunityTriplet = (CCheckBoxUI *)m_PaintManager.FindControl(
      _T("RULE_WITH_LABEL_COMMUNITY_TRIPLET"));
  m_boxQuestion =
  ...
  m_optionSortLower = (COptionUI *)m_PaintManager.FindControl(
      _T("SORT_WITH_CERATE_TIME_LOWER"));
  m_optionSortHigher = (COptionUI *)m_PaintManager.FindControl(
      _T("SORT_WITH_CERATE_TIME_HIGHER"));
...
void TestFrame::OnGetData() {
  ...
  if (gb.GetData(SearchCondition{search_type, search_type_message}, cdtList,
                 szFilePath, 50, filterConditions)) {
...
void TestFrame::GetFilterConditions(ConditionList &conditions) {
  ...
  char *szFilterCondition =
      new char[filterCondition.length() + strlen(";") + 1];
...
void TestFrame::GetCondition(const std::string &strCondition, Condition &cdt) {
  ...
  if (dot != std::string::npos) {
    ...
    strRealCondition =
        strCondition.substr(next, strCondition.length() - (next));
