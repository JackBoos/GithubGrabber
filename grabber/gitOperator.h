...
namespace GithubGrabber
{
    typedef struct _gitRemote
    {
        _gitRemote()
        {
            memset(this, 0, sizeof(_gitRemote));
        }
        ~_gitRemote() {}
        bool isEmpty()
        {
            return szLocation[0] == '\0' && szRemoteUrl[0] == '\0';
        }
        void clear()
        {
            memset(this, 0, sizeof(_gitRemote));
    ...
    public:
        ...
        bool AddRemote(std::string remote, gitRemote& outRemote, gitRemote& originRemote = gitRemote());
    ...
    private:
        ...
        bool execGit(std::string command, gitRemote& remote, const char* workingDirectory = NULL);
