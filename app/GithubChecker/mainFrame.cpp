...
CDuiString TestFrame::GetSkinFolder() { return _T(""); }

CDuiString TestFrame::GetSkinFile() { return _T("checkerMainFrame.xml"); }

LPCTSTR TestFrame::GetWindowClassName(void) const { return _T("MainFrame"); }
...
void TestFrame::InitWindow() {
  ...
  m_editFilterDay =
      (CEditUI *)m_PaintManager.FindControl(_T("FILTER_DAY_OVER"));
  m_editComment =
      (CRichEditUI *)m_PaintManager.FindControl(_T("COMMENT_CONTENT"));
  m_editOutput = (CRichEditUI *)m_PaintManager.FindControl(_T("OUTPUT"));
  m_optionFilterActiveDay =
      (COptionUI *)m_PaintManager.FindControl(_T("FILTER_ACTIVE_DAY"));
  m_optionFilterCreateDay =
      (COptionUI *)m_PaintManager.FindControl(_T("FILTER_CREATE_DAY"));
  m_optionFilterLastCommitDay =
      (COptionUI *)m_PaintManager.FindControl(_T("FILTER_COMMIT_DAY"));
  m_optionIssue =
      (COptionUI *)m_PaintManager.FindControl(_T("SEARCH_TYPE_ISSUE"));
  m_optionPR = (COptionUI *)m_PaintManager.FindControl(_T("SEARCH_TYPE_PR"));
  m_boxPortBug =
      (CCheckBoxUI *)m_PaintManager.FindControl(_T("RULE_WITH_LABEL_PORT_BUG"));
  m_boxPortFeature = (CCheckBoxUI *)m_PaintManager.FindControl(
      _T("RULE_WITH_LABEL_PORT_FEATURE"));
  m_boxQuestion =
      (CCheckBoxUI *)m_PaintManager.FindControl(_T("RULE_WITH_LABEL_QUESTION"));
  m_boxNeedsRepro = (CCheckBoxUI *)m_PaintManager.FindControl(
      _T("RULE_WITH_LABEL_NEEDS_REPRO"));
  m_boxNeedMoreInfo = (CCheckBoxUI *)m_PaintManager.FindControl(
      _T("RULE_WITH_LABEL_NEED_MORE_INFO"));
  m_boxDocument =
      (CCheckBoxUI *)m_PaintManager.FindControl(_T("RULE_WITH_LABEL_DOCUMENT"));
  m_boxVcpkgBug = (CCheckBoxUI *)m_PaintManager.FindControl(
      _T("RULE_WITH_LABEL_VCPKG_BUG"));
  m_boxVcpkgFeature = (CCheckBoxUI *)m_PaintManager.FindControl(
      _T("RULE_WITH_LABEL_VCPKG_FEATURE"));
  m_boxReviewed =
      (CCheckBoxUI *)m_PaintManager.FindControl(_T("RULE_WITH_LABEL_REVIEWED"));

  std::wstring filepath = GetCurrentPath() + _T("result.xlsx");
  m_editFilePath->SetText(filepath.c_str());
}

LRESULT TestFrame::OnClose(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/,
                           BOOL &bHandled) {
  PostQuitMessage(0);
  return 0;
}
...
void TestFrame::OnGetData() {
  ...
  if (m_iFilterDay < 0) {
    ::MessageBox(NULL, _T("Please input a valid filter day number."), _T(""),
                 NULL);
    return;
  }
  ...
  SearchType search_type =
      bIsIssue ? SearchType::SEARCHTYPE_ISSUE : SearchType::SEARCHTYPE_PR;
  const char *search_type_message = bIsIssue ? SEARCHNAME_ISSUE : SEARCHNAME_PR;

  if (!(*szFilePath)) {
    ::MessageBox(NULL, _T("Please select the output file first."), _T(""),
                 NULL);
    return;
  }
  ...
  if (!m_gb) {
    m_gb =
        new GithubGrabber::grabber(GITHUB_SEARCH_BASE_URL, szUserName, szToken);
    m_gb->Init();
  }
  ...
  if (!m_gb->GetData(SearchCondition{search_type, search_type_message}, cdtList,
                     szFilePath, 50, ConditionList(), &m_dataList, false)) {
...
void TestFrame::OnSendComment() {
  ...
  if (!m_gb) {
    m_gb = new GithubGrabber::grabber(GITHUB_SEARCH_OPERATE_URL, szUserName,
                                      szToken);
    m_gb->Init();
  }
...
void TestFrame::OnGetToken() {
  ::MessageBox(NULL,
               _T("Use token will have permission to edit issue/PR state, ")
               _T("please notice that!"),
               _T("Warning!"), NULL);
  OPENFILENAME ofn;
  TCHAR szFile[MAX_PATH] = _T("");
...
void TestFrame::FilterData() {
  ...
  for (auto i = m_dataList.begin(); i != m_dataList.end();) {
    ...
    if (bUseActiveDay) {
      idays = GetActiveDay(
          *(std::string *)find(i->begin(), i->end(), "updated_at")->value);
    } else {
      idays = GetActiveDay(
          *(std::string *)find(i->begin(), i->end(), "created_at")->value);
    }
    ...
    if (idays > m_iFilterDay) {
      ...
      swprintf(szNumber, 16, L"%d",
               *(int *)find(i->begin(), i->end(), "number")->value);
      outData += desc;
      outData += _T(": ");
      outData += szNumber;
      outData += _T(" outdated ");
...
int TestFrame::GetActiveDay(std::string &from) {
  ...
  int *date[6] = {&(tmFromTime.tm_year), &(tmFromTime.tm_mon),
                  &(tmFromTime.tm_mday), &(tmFromTime.tm_hour),
                  &(tmFromTime.tm_min),  &(tmFromTime.tm_sec)};
  int i = 0;
  ...
  int abs =
      ((int)(nowTime->tm_year - tmFromTime.tm_year - (bOverMon ? 1 : 0))) *
          ONE_YEAR_DAY +
      ((int)(nowTime->tm_mon - tmFromTime.tm_mon + (bOverMon ? 12 : 0) -
             (bOverDay ? 1 : 0))) *
          ONE_MON_DAY +
...
std::string TestFrame::GetComment() {
  ...
  strCommentContent =
      regex_replace(strCommentContent, regex("\\\\n"), std::string("\n"));
