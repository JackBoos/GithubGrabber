...
bool excelExecuter::WriteToFile(const char* filePath, const ItemsDataList& dataList, const ConditionList& filterConditions)
{
...
void excelExecuter::TraverseHeader(const OneItemDataList& dataList, const xlsxiowriter writer, const std::vector<std::string>& filter)
{
    for (auto i = dataList.begin(); i != dataList.end(); i++)
    {
        switch (i->type)
        ...
        default:
        {
            ...
            if (!currentName.empty())
            {
                ...
                {
                    xlsxiowrite_add_column(writer, currentName.c_str(), DEFAULT_COLUMN_LENGTH);
                }
            }
        }
        break;
...
void excelExecuter::TraverseSubHeader(const void* dataList, const xlsxiowriter writer, const std::vector<std::string>& filter, const char* originName)
{
    if (!dataList)
    {
        return;
...
void excelExecuter::TraverseData(const OneItemDataList& dataList, const xlsxiowriter writer, const std::vector<std::string>& filter)
{
...
void excelExecuter::TraverseSubData(const void* dataList, const xlsxiowriter writer, const std::vector<std::string>& filter, const char* originName)
{
    if (!dataList)
    {
        return;
...
std::string excelExecuter::GetIteamName(const ItemData& dataList, const char* prevString)
{
    ...
    case DataType::DATATYPE_ARRAY:
    {
        return GetIteamName((const ItemData&)dataList.value, strFullString.c_str());
    }
    break;
    default:
    break;
...
std::string excelExecuter::GetFliterName(const Condition& cdt, const char* prevString)
{
    std::string strFullString;
    if (prevString && *prevString)
    {
