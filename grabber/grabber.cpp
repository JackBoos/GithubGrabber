...
bool GithubGrabber::grabber::GetPerIssueOrPR(const std::string &number,
                                             ItemsDataList *outDataList) {
...
bool grabber::GetData(SearchCondition &search, const ConditionList &conditions,
                      const char *outFile, unsigned int onePageCount,
                      const ConditionList &filterConditions,
                      ItemsDataList *outDataList, bool bSave) {
  ...
  std::string strCurrentUrl =
      strSubUrl + PAGE_KEYWORD + "1" + PER_PARGE_KEYWORD + szOnePageSize;
  bSuc = m_parser->GetData(strCurrentUrl.c_str(), outData);
  if (!bSuc)
    return bSuc;

  int iTotalItemSize = dataAnalysis::GetInstance().GetTotalSize(
      outData, GITHUB_TOTAL_COUNT_NAME);
  ItemsDataList vData;
  bSuc =
  ...
  // Start from page 2
  const int getPage = ((int)((float)iTotalItemSize / (float)onePageCount) >
                       (iTotalItemSize / onePageCount))
                          ? (iTotalItemSize / onePageCount)
                          : (iTotalItemSize / onePageCount + 1);
  for (int i = 2; i <= getPage; i++) {
    char szPageNum[4] = {0};
    sprintf(szPageNum, "%d", i);
    outData.clear();
    strCurrentUrl = strSubUrl + PAGE_KEYWORD + szPageNum + PER_PARGE_KEYWORD +
                    szOnePageSize;
    m_parser->GetData(strCurrentUrl.c_str(), outData);
    bSuc = dataAnalysis::GetInstance().ParseData(outData, GITHUB_ITEAM_NAME,
                                                 vData);
    if (!bSuc)
      return bSuc;
  }
...
bool grabber::Execute(const std::string &repo, SearchType type,
                      const ExecuteData &data) {
  if (repo.empty() || type == SearchType::SEARCHTYPE_UNKNOWN ||
      data.type == ExecuteType::EXECUTE_TYPE_UNKNOWN)
    return false;
  ...
  case ExecuteType::EXECUTE_TYPE_CLOSE_ISSUE_PR: {
    return m_parser->PostData(subUrl.c_str(), GITHUB_CLOSE_ISSUE_PR_BODY,
                              GITHUB_OP_PATCH);
  } break;
  default:
    return false;
...
bool GithubGrabber::grabber::ExecuteBaseGet(const std::string &url,
                                            std::string &data) {
  return m_parser->GetData(url.c_str(), data);
}

bool GithubGrabber::grabber::ExecuteBasePost(const std::string &url,
                                             std::string &data) {
  return m_parser->PostData(url.c_str(), data);
}
